...
CodeJitter::CodeJitter() : pattern_sync_each_ref(false), flushing_strategy(FLUSHING_STRATEGY::EARLIEST_POSSIBLE), fencing_strategy(FENCING_STRATEGY::LATEST_POSSIBLE), total_activations(5000000), num_aggs_for_sync(2) {
...
void CodeJitter::jit_strict(int num_acts_per_trefi, FLUSHING_STRATEGY flushing, FENCING_STRATEGY fencing, const std::vector<volatile char *> &aggressor_pairs, bool sync_each_ref, int num_aggressors_for_sync, int total_num_activations) {
    ...
    if (static_cast<size_t>(NUM_TIMED_ACCESSES) > aggressor_pairs.size()) {
        Logger::log_error(format_string("NUM_TIMED_ACCESSES (%d) is larger than #aggressor_pairs (%zu).", NUM_TIMED_ACCESSES, aggressor_pairs.size()));
        return;
    }
    ...
    for (int i = NUM_TIMED_ACCESSES; i < static_cast<int>(aggressor_pairs.size()) - NUM_TIMED_ACCESSES; i++) {
        ...
        */
        ...
        a.mov(asmjit::x86::rax, cur_addr);
        //a.mov(asmjit::x86::rdi, asmjit::x86::ptr(asmjit::x86::rax));
        ...
        */
        ...
        if (sync_each_ref && ((cnt_total_activations % num_acts_per_trefi) == 0)) {
            std::vector<volatile char *> aggs(aggressor_pairs.begin() + i, std::min(aggressor_pairs.begin() + i + NUM_TIMED_ACCESSES, aggressor_pairs.end()));
            sync_ref(aggs, a);
        }
    }
...
void to_json(nlohmann::json &j, const CodeJitter &p) {
    j = {{"pattern_sync_each_ref", p.pattern_sync_each_ref}, {"flushing_strategy", to_string(p.flushing_strategy)}, {"fencing_strategy", to_string(p.fencing_strategy)}, {"total_activations", p.total_activations}, {"num_aggs_for_sync", p.num_aggs_for_sync}};
}
